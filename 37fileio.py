...
# 간단한 인삿말이 저장된 hello2.dat 파일에서도 읽어오기
# 간단한 인삿말을 파일에서 읽어오기
...
with open('member.dat', encoding='UTF-8') as f:
    doc6 = []
    doc6 = f.readline()  # 행 단위로 하나씩만 읽어 리스트로 반환
print(doc6)  # 한건만 출력됨(반복문으로 처리해야 함)
...
with open('member.dat', encoding='UTF-8') as f:
    ...
    while True:
        ...
        line = f.readlines()               # 파일로부터 한줄 읽어서
        if not line:                       # 읽은 내용이 없으면 반복 중지
            break                          # 읽은 내용이 있다면 리스트에 저장
            doc7.append(line)
print(doc7)  # 모두 출력
